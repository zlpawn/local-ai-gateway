...
export function selectExposedEndpoints(endpoints = []) {
  const ordinary = endpoints.filter((endpoint) => endpoint?.purpose !== "vision_fallback");
...
export function validateGatewayConfig(
  ...
  for (const [clientName, client] of Object.entries(config?.clients || {})) {
    ...
    const ordinaryEndpoints = endpoints.filter((endpoint) => endpoint?.purpose !== "vision_fallback");
