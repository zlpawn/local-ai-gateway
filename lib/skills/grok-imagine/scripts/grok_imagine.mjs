...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log(`[Grok Imagine Skill] Starting ${type} generation...`);
  try {
    if (type === "video") {
      const result = await generateVideo({ prompt, imagePath, aspectRatio });
      console.log(`\nSUCCESS:\n${result.markdown}`);
    } else {
      const result = await generateImage({ prompt, imagePath, aspectRatio });
      console.log(`\nSUCCESS:\n${result.markdown}`);
    }
  } catch (err) {
    console.error(`\nERROR: ${err.message}`);
