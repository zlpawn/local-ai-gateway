...
export async function generateImage(options = {}) {
  ...
  // POSIX absolute path for universal Markdown rendering across Antigravity/Codex Desktop/Claude
...
export async function checkAndDownloadVideo(requestId, options = {}) {
  ...
  for (let i = 1; i <= maxPolls; i++) {
    ...
    console.log(`[Grok Imagine Video] 轮询进度 (${i}/${maxPolls}) [RequestID: ${requestId}] - 状态: ${status} ${progress}`);
    ...
    if (status === "done" || status === "completed" || pollRes.video_url || pollRes.url) {
      ...
      throw new Error(`[RequestID: ${requestId}] 视频生成失败: ${pollRes.error || "服务端处理失败"}`);
    }
...
export async function generateVideo(options = {}) {
  ...
  if (!requestId) {
    throw new Error("视频任务提交失败：服务端响应缺少 request_id");
  }
  console.log(`[Grok Imagine Video] 任务创建成功! Request ID: "${requestId}"`);

  // 2. Poll Task Status via checkAndDownloadVideo helper to ensure Request ID recovery
  try {
    return await checkAndDownloadVideo(requestId, { ...options, prompt, outputDir });
  } catch (err) {
    // Preserve Request ID in Error message for resilient task recovery
