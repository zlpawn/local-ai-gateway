...
/**
 * Robust Media File Downloader with Node fetch & System curl fallback
 */
export async function downloadMediaFile(url, targetPath) {
  ...
  try {
    ...
    console.warn(`[Grok Media Download] Node fetch 遇到网络限制 (${err.message})，正自动唤起系统 curl 高可用下载...`);
  }

  // 2. Fallback to System curl for signed CDN URLs
  try {
    const escapedUrl = url.replace(/"/g, '\\"');
    const escapedTarget = targetPath.replace(/"/g, '\\"');
    execSync(`curl -sSL --connect-timeout 15 -m 120 "${escapedUrl}" -o "${escapedTarget}"`, { windowsHide: true });
    if (fs.existsSync(targetPath) && fs.statSync(targetPath).size > 0) {
      return targetPath;
    }
  } catch (curlErr) {
    throw new Error(`系统 curl 媒体资源下载失败 (${url}): ${curlErr.message}`);
  }

  throw new Error(`无法下载媒体文件 (${url}): 资源响应为空`);
}
...
export async function checkAndDownloadVideo(requestId, options = {}) {
  ...
  try {
    ...
    throw new Error(`无法从 CDN 下载视频资源 (${videoUrl}): ${err.message}`);
  }
  ...
  return {
    ...
    filename,
    prompt,
    markdown: `![Generated Video](${absPath})\n\n[▶️ 点击播放/预览视频](${fileUrl}) | [📁 打开文件](${fileUrl})`,
  };
}
...
export async function generateVideo(options = {}) {
  ...
  if (options.dryRun) {
    ...
    return {
      ...
      prompt,
      markdown: `![Generated Video (DRY-RUN)](${path.join(outputDir, generateSemanticFilename(prompt, "mp4", options.filename))})`,
    };
  }
...
export function parseCliArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--help" || arg === "-h") {
      ...
      result.duration = Math.floor(parseFloat(next)) || 6;
      i++;
