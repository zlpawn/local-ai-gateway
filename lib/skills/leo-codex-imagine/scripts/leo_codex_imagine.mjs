...
export function parseCliArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (key === "--image" || key === "--images") { args.imagePaths = val; i++; }
    else if (key === "--aspect-ratio") { args.aspectRatio = val; i++; }
    else if (key === "--size") { args.size = val; i++; }
...
export function generateSemanticFilename(prompt, ext, prefix = "codex", explicitFilename = null) {
...
function showHelp() {
  console.log(`leo-codex-imagine - 通过网关 Codex 订阅节点生成图片

用法:
  node leo_codex_imagine.mjs --prompt "描述" [options]

参数:
  --prompt         图片描述提示词（必填）
  --images         参考图路径，逗号分隔
  --aspect-ratio   画面比例 (默认: auto)
  --size           图片尺寸 (默认: auto)
...
async function main() {
  ...
  const filename = args.filename || generateSemanticFilename(args.prompt, "png", "codex");
  ...
  if (args.dryRun) {
    ...
    console.log("  size:", args.size || "auto");
    console.log("  quality:", args.quality || "medium");
  ...
  if (args.imagePaths) {
    body.image_paths = args.imagePaths.split(",").map(s => s.trim()).filter(Boolean);
  }
  ...
  // Gateway already persists to disk and returns file_path
