...
description: 使用火山引擎方舟 ARK_API_KEY 调用豆包 Seedance 2.0 进行文生视频 / 图生视频，并通过任务轮询自动下载 MP4。
---

# Huoshan Imagine Skill (火山引擎视频生成)
...
## 输出目录与文件命名
1. **默认存储路径**: 当前工作区的 `./videos/` 目录下。
2. **格式规约**: `volcano_<提示词缩写>_<YYYYMMDDHHmmss>.mp4`，例如 `volcano_cyberpunk_night_20260731203015.mp4`，自动防重且可读。
