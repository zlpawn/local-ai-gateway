export class ResponsesWriter {
  constructor({ model, emit, responseId = `resp_${Date.now()}`, onCompleted }) {
  ...
  completed(usage) {
    if (typeof this.onCompleted === "function") { try { this.onCompleted(usage, this.response("completed")); } catch {} }
    if (this.terminal) return;
...
export function unwrapCustomInput(argumentsText) {
  return normalizeCustomInput(argumentsText).input;
}

export function normalizeCustomInput(argumentsText) {
  const text = typeof argumentsText === "string"
    ? argumentsText
  ...
  try {
    ...
    return { input: text, fallback: false };
  }

  if (typeof parsed === "string") {
    return { input: parsed, fallback: false };
  }
  if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
    if (typeof parsed.input === "string") {
      return { input: parsed.input, fallback: false };
    }

    const values = Object.values(parsed);
    if (values.length === 1) {
      const nested = normalizeCustomInput(JSON.stringify(values[0]));
  ...
  return {
    input: text,
    fallback: true,
    shape: customInputShape(parsed),
  };
}

function customInputShape(value) {
