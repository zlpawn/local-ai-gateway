...
export function sanitizeResponsesInput(source) {
  ...
  if (Array.isArray(source.input)) {
    result.input = source.input
      .map((item) => {
        ...
        const isMessage = item.type === "message" || (
          item.type == null && (item.role === "user" || item.role === "assistant" || item.role === "system" || item.role === "developer")
        );
        if (isMessage) {
          const role = item.role === "assistant" ? "assistant" : item.role === "system" || item.role === "developer" ? "system" : "user";
          const content = sanitizeContent(item.content, role);
    ...
    while (
      ...
      && result.input[result.input.length - 1]?.type === "message"
      && result.input[result.input.length - 1]?.role === "assistant"
    ) {
      result.input.pop();
    }
  }
...
function sanitizeContent(content, role = "user") {
  ...
  return content
    .map((part) => {
      ...
      if (part.type === "image_url" || part.type === "input_image") {
        const imageUrl = typeof part.image_url === "string" ? part.image_url : part.image_url?.url || part.url || "";
