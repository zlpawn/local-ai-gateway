...
export function createTaskQueue({ store, registry, concurrency = 2, maxRetries = 1, retryBackoffMs = 2000 } = {}) {
  ...
  async function executeTask(task) {
    ...
    try {
      console.error("[task-queue] executing task:", task.id, "type:", task.type);
      ...
      if (store.isCancelRequested(task.id)) {
        ...
        console.error("[task-queue] task succeeded:", task.id);
      ...
      console.error("[task-queue] task error:", task.id, "retries:", task.retries, "error:", err.message, "stack:", err.stack);
      // Retry logic - use task.retries from the in-memory object, not the DB
      if (task.retries < maxRetries) {
        const backoff = retryBackoffMs * Math.pow(2, task.retries);
        console.error("[task-queue] retrying in", backoff, "ms, attempt", task.retries + 1, "/", maxRetries);
        const waited = await abortableWait(backoff, () => shuttingDown || store.isCancelRequested(task.id));
        if (waited.aborted) {
          store.finalizeCancelled(task.id);
          return;
        }
        store.updateProgress(task.id, 0, `retrying (${task.retries + 1}/${maxRetries})`);
        activeControllers.delete(task.id);
        // Re-run with incremented retries counter (in-memory, not DB)
        executeTask({ ...task, retries: task.retries + 1 });
        return;
      }
      console.error("[task-queue] task FAILED (no more retries):", task.id);
...
function abortableWait(ms, shouldAbort) {
