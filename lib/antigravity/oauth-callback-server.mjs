...
export async function startCallbackServerOnFreePort({
  ...
  const port = await findFreePort(preferredPort, { host, attempts });
  const codePromise = startCallbackServer({ port, state, pathPrefix, host, timeoutMs });
  // Give the server a brief moment to bind; if bind fails, codePromise rejects quickly.
