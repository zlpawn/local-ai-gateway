...
export function getStoragePath(env = process.env) {
  ...
  return join(homedir(), ".shrimp", "antigravity-signatures.json");
}
...
export function getSignature(sessionFp, callId) {
  ...
  // 1. Primary lookup under requested sessionFp by callId
  if (session) {
  ...
  // 2. Secondary fallback lookup under _default session by callId
  if (fp !== "_default") {
...
// Retrieve any active signature from the session or global cache as a fallback
// for historical tool calls that missed caching on prior turns.
export function getFallbackSignature(sessionFp) {
  loadFromDisk();
  const now = Date.now();
  const fp = sessionFp || "_default";

  // 1. Try any active signature in the requested session.
  const session = sessions.get(fp);
  if (session) {
    for (const [, entry] of session) {
      if (entry && typeof entry.sig === "string" && now - entry.ts <= TTL_MS) {
        return entry.sig;
      }
    }
  }

  // 2. Only fall back to the _default bucket when the request itself has no
  // usable fingerprint. We deliberately do NOT scan other sessions' signatures
  // here: a thoughtSignature is bound to the specific function call that
  // produced it, and injecting a foreign session's signature onto a different
  // call_id causes the v1internal backend to reject with gRPC status=3
  // ("Function call is missing a thought_signature" / signature mismatch),
  // which is exactly the retry storm we are fixing.
  if (fp !== "_default") {
    const defaultSession = sessions.get("_default");
    if (defaultSession) {
      for (const [, entry] of defaultSession) {
        if (entry && typeof entry.sig === "string" && now - entry.ts <= TTL_MS) {
          return entry.sig;
        }
      }
