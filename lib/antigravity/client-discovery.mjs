...
// Preferred Antigravity desktop client identity observed in AG-Manager / local installs.
const PREFERRED_CLIENT_ID =
  "9999999999-fakeclientid0fortesting0.apps.googleuser.test";
const PREFERRED_CLIENT_SECRET = "FAKESEC-aaaaaaaaaaaaaaaaaaaaaaaaxAAA";
...
export function chooseCredentialPair(clientIds, clientSecrets) {
  const ids = unique(clientIds);
  const secrets = unique(clientSecrets);
  if (!ids.length || !secrets.length) return null;

  const preferredId = ids.includes(PREFERRED_CLIENT_ID) ? PREFERRED_CLIENT_ID : ids[0];
  const preferredSecret = secrets.includes(PREFERRED_CLIENT_SECRET)
...
export function discoverAntigravityClientCredentials({
  ...
  const pair = chooseCredentialPair(allIds, allSecrets);
  if (!pair) {
    return {
      ok: false,
