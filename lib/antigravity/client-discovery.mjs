...
const CLIENT_ID_RE = /[0-9]{6,}-[a-z0-9]{10,}\.(?:apps\.googleusercontent\.com|apps\.googleuser\.test)/g;
const SECRET_PREFIXES = ["FAKESEC-", "FAKESEC-"];
...
export function extractClientSecretsFromText(text) {
  ...
  for (const prefix of SECRET_PREFIXES) {
    ...
    while (index < source.length) {
      const start = source.indexOf(prefix, index);
      if (start < 0) break;
      let body = "";
      ...
      while (cursor < source.length && body.length < BODY_LEN && isSecretBodyChar(source[cursor])) {
        ...
        if (SECRET_PREFIXES.some((p) => source.startsWith(p, cursor))) break;
        body += source[cursor];
        cursor += 1;
      }
