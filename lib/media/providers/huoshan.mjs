...
export const huoshanAdapter = {
  ...
  async generateImage(options, ctx) {
    ...
    if (Array.isArray(options.imageUrls) && options.imageUrls.length) body.image = options.imageUrls;
  ...
  async createVideoTask(options, ctx) {
    ...
    if (Array.isArray(options.imageB64List)) {
      for (const b64 of options.imageB64List) {
        content.push({ type: "image_url", image_url: { url: `data:image/jpeg;base64,${b64}` }, role: "first_frame" });
      }
    }
    const body = {
