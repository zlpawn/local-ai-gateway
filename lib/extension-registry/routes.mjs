...
export function routeExtensionRequest(req, res, _context, reqPath, deps) {
  ...
  if (reqPath === "/v1/extensions/download" && req.method === "GET") {
    const cookieExtDir = path.join(extensionsDir, "leo-cookie-txt-locally");
    if (!fs.existsSync(cookieExtDir)) {
      sendJson(res, 404, { error: { type: "not_found", message: "Extension package not found." } });
      return;
    }
    const tmpZip = path.join(os.tmpdir(), `leo-cookie-txt-locally-${Date.now()}.zip`);
    try {
      if (process.platform === "win32") {
        execSync(`powershell -NoProfile -Command "Compress-Archive -Path '${cookieExtDir}\\*' -DestinationPath '${tmpZip}' -Force"`, { windowsHide: true });
      } else {
        execSync(`cd "${cookieExtDir}" && zip -r "${tmpZip}" .`, { stdio: "ignore" });
      }
      const zipBuf = fs.readFileSync(tmpZip);
      res.writeHead(200, {
        "Content-Type": "application/zip",
        "Content-Disposition": 'attachment; filename="Leo-cookie.txt-Locally.zip"',
...
export function routeCookieImport(req, res, _context, deps) {
  ...
  return readJsonBody(req)
    .then((body) => {
      ...
      const primaryDomain = (body.domain || domains[0] || "").replace(/[^a-zA-Z0-9.-]/g, "_");
