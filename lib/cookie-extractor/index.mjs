...
const BROWSER_PROFILES = {
  chrome: {
    name: "Google Chrome",
    // Newer Chrome moved Cookies into a Network/ subdirectory. Candidate paths in priority order.
    darwin: (p) => [
      path.join(p, "Library/Application Support/Google/Chrome/Default/Network/Cookies"),
      path.join(p, "Library/Application Support/Google/Chrome/Default/Cookies"),
    ],
    win32: (p) => [
      path.join(p, "AppData/Local/Google/Chrome/User Data/Default/Network/Cookies"),
      path.join(p, "AppData/Local/Google/Chrome/User Data/Default/Cookies"),
    ],
    linux: (p) => [
      path.join(p, ".config/google-chrome/Default/Network/Cookies"),
      path.join(p, ".config/google-chrome/Default/Cookies"),
    ],
  ...
  edge: {
    name: "Microsoft Edge",
    darwin: (p) => [
      path.join(p, "Library/Application Support/Microsoft Edge/Default/Network/Cookies"),
      path.join(p, "Library/Application Support/Microsoft Edge/Default/Cookies"),
    ],
    win32: (p) => [
      path.join(p, "AppData/Local/Microsoft/Edge/User Data/Default/Network/Cookies"),
      path.join(p, "AppData/Local/Microsoft/Edge/User Data/Default/Cookies"),
    ],
    linux: (p) => [
      path.join(p, ".config/microsoft-edge/Default/Network/Cookies"),
      path.join(p, ".config/microsoft-edge/Default/Cookies"),
    ],
  ...
  brave: {
    name: "Brave Browser",
    darwin: (p) => [
      path.join(p, "Library/Application Support/BraveSoftware/Brave-Browser/Default/Network/Cookies"),
      path.join(p, "Library/Application Support/BraveSoftware/Brave-Browser/Default/Cookies"),
    ],
    win32: (p) => [
      path.join(p, "AppData/Local/BraveSoftware/Brave-Browser/User Data/Default/Network/Cookies"),
      path.join(p, "AppData/Local/BraveSoftware/Brave-Browser/User Data/Default/Cookies"),
    ],
    linux: (p) => [
      path.join(p, ".config/BraveSoftware/Brave-Browser/Default/Network/Cookies"),
      path.join(p, ".config/BraveSoftware/Brave-Browser/Default/Cookies"),
    ],
...
export function detectBrowsers() {
  ...
  for (const [id, config] of Object.entries(BROWSER_PROFILES)) {
    if (id === "firefox") {
      ...
      // Chrome-family: config[platform] returns candidate paths (new Network/ path first, legacy fallback).
      const candidates = config[platform]?.(home);
      const cookieDb = Array.isArray(candidates)
        ? candidates.find((c) => fs.existsSync(c))
        : (candidates && fs.existsSync(candidates) ? candidates : null);
      if (cookieDb) {
...
function openCookieDb(cookieDbPath) {
  // Browser may have the DB locked. Copy to a temp file so we can open it read-only.
  ...
  try {
    ...
    // On Windows, Chrome/Edge/Brave hold the Cookies SQLite file with an
    // exclusive lock (FileShare.None) while running. No user-mode reader
    // (fs, .NET FileShare.ReadWrite, esentutl /y) can open it; only a
    // Volume Shadow Copy (VSS) can bypass the lock, which requires admin.
    // macOS/Linux allow concurrent reads, so this never trips there.
    // Try VSS via esentutl as a best-effort fallback (works only when the
    // gateway runs elevated); otherwise surface a clear, actionable error
    // instead of the raw EBUSY.
    if (err && (err.code === "EBUSY" || err.code === "EPERM" || err.code === "EACCES") && process.platform === "win32") {
      try {
        execSync(`esentutl /y /vss "${cookieDbPath}" /d "${tmpDb}" /o`, { stdio: "ignore", windowsHide: true });
        copyOk = true;
      } catch {
        /* VSS unavailable (non-admin) - fall through to friendly error */
      }
    }
    if (!copyOk) {
      throw new Error(
        "无法读取浏览器 Cookies 数据库：文件被浏览器独占锁定（EBUSY）。\n" +
        "在 Windows 上，Chrome/Edge/Brave 运行时会独占锁定 Cookies 文件，无法在浏览器开启时读取。\n" +
        "请完全关闭对应的浏览器后重试。"
      );
    }
  }
  const db = new DatabaseSync(tmpDb, { readOnly: true });
  // readOnly DB cannot set WAL (write op); read queries need no journal mode change.
...
function getChromeKeyMacOS(keychainKey) {
  ...
  let password;
  try {
    password = execSync(`security find-generic-password -wa "${keychainKey}"`, { encoding: "utf8", windowsHide: true }).trim();
  } catch {
...
function getChromeKeyWindows(localStateRelPath) {
  ...
  let decryptedHex;
  try {
    decryptedHex = execSync(`powershell -NoProfile -Command "${script}"`, { encoding: "utf8", maxBuffer: 1024 * 1024, windowsHide: true }).trim();
  } catch {
...
function getChromeKeyLinux() {
  ...
  if item.get_label()=='Chrome Safe Storage': print(item.get_secret().decode()); break"`, { encoding: "utf8", timeout: 5000, windowsHide: true }).trim();
...
function readChromeCookies(db, key, domainFilter) {
  let query = "SELECT host_key, path, name, value, encrypted_value, is_secure, is_httponly, CAST(expires_utc AS TEXT) AS expires_utc FROM cookies";
  const params = [];
  if (domainFilter) {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      expires: Number(BigInt(row.expires_utc) / 1000000n - 11644473600n), // Chrome epoch (microseconds since 1601) to Unix seconds
    };
  }).filter((c) => c.value !== "");
}
