...
export function addClient({ configPath, secretsPath, client, copyFrom, mode = "replace", dryRun = false }) {
  if (!client) {
  ...
  state.config.clients = {
    ...(state.config.clients || {}),
    [client]: state.config.clients?.[client] || { endpoints: [] },
  };
  if (!copyFrom) {
    saveState({ configPath, secretsPath, config: state.config, secrets: state.secrets, dryRun });
    return { client, created: true, dry_run: Boolean(dryRun) };
  }
  return copyClient({ configPath, secretsPath, from: copyFrom, to: client, mode, dryRun });
}
