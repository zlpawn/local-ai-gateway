...
export function detectDataDir(packageRoot, { cwd = process.cwd(), env = process.env } = {}) {
  ...
  const preferred = path.join(os.homedir(), DEFAULT_DATA_DIR_NAME);
  const legacy = path.join(os.homedir(), LEGACY_DATA_DIR_NAME);
  if (fs.existsSync(preferred) || !fs.existsSync(legacy)) return preferred;
  // transitional: if only legacy exists and preferred doesn't, keep reading legacy until rename completes
  return legacy;
}
