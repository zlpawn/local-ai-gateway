 ...
 *
 * Built-in nodes: fetch_info, agent_reach_get, download_audio, download_video, transcribe,
 *                 chunk, vectorize
 *
...
const DEFAULT_NODES = [
  ...
  },
  {
    ...
    weight: 0.10,
    async run(ctx, { signal, onProgress }) {
      // Skip if user doesn't want to keep the original video, or if agent-reach provided content
      if (ctx.keepVideo === false || ctx.skipDownload) return { videoPath: null };
  ...
  },
  {
    ...
    weight: 0.50,
    async run(ctx, { signal, onProgress }) {
      if (ctx.skipTranscribe) {
        onProgress?.(1.0, "使用 Agent Reach 提供的内容");
        return {};
      }
 ...
 */
/**
 * Convert plain text (from agent-reach) to segments with estimated timestamps.
 * No real timestamps available, so we estimate based on average speaking rate.
 */
function textToSegments(text) {
  ...
  const segments = [];
  const avgCharsPerSecond = 4; // rough estimate for mixed Chinese/English
  let currentTime = 0;
...
export async function runVideoKbPipeline(payload, { signal, onProgress, onSteps } = {}) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    console.log(`[video-kb] pipeline: completed node "${node.id}"`);
      ...
      console.error("[video-kb] node FAILED:", node.id, "error:", err.message);
    console.log(`[video-kb] pipeline: failed node "${node.id}":`, err.message);
