...
export function detectWhisperTools() {
  ...
  for (const tool of WHISPER_TOOLS) {
    ...
    try {
      ...
      const toolPath = execSync(whichCmd, { encoding: "utf8", timeout: 3000, windowsHide: true }).trim().split("\n")[0];
      if (!toolPath) continue;

      let version = "";
      try {
        version = execSync(`${tool.command} --version`, { encoding: "utf8", timeout: 3000, windowsHide: true }).trim();
...
function transcribeMlxWhisper(audioPath, { tool, modelSize, language, outputDir, baseName, signal, onProgress }) {
  return new Promise((resolve, reject) => {
    ...
    runWhisperProcess(args, { signal, onProgress, reject, outputDir, baseName, resolve: async () => {
...
function runWhisperProcess(args, { signal, onProgress, reject, resolve, outputDir, baseName }) {
  const command = args[0];
  const rest = args.slice(1);
  console.log("[video-kb] transcribe: spawning", command, rest.join(" "));
  console.error("[whisper] spawning:", command, rest.join(" "));
  const proc = spawn(command, rest, { timeout: 0, windowsHide: true });
  let stderr = "";
  let resolved = false;
  let outputWatcher = null;

  proc.stdout.on("data", (d) => {
    console.log("[video-kb] transcribe stdout:", d.toString().trim());
  });

  // On Windows, whisper-ctranslate2 sometimes hangs after writing the output file.
  // Watch for the output file and force-exit the process once it appears.
  if (outputDir && baseName) {
    const expectedJson = path.join(outputDir, `${baseName}.json`);
    console.log("[video-kb] transcribe: watching for output", expectedJson);
    outputWatcher = setInterval(() => {
      if (resolved) return;
      try {
        console.error("[whisper] output file detected:", expectedJson);
          if (fs.existsSync(expectedJson)) {
          console.log("[video-kb] transcribe: output file detected, killing process in 2s");
          clearInterval(outputWatcher);
          outputWatcher = null;
          setTimeout(() => {
            if (!resolved) {
              console.log("[video-kb] transcribe: force killing process (was hanging)");
  ...
  proc.stderr.on("data", (d) => {
    ...
    console.log("[video-kb] transcribe stderr:", text.trim());
  ...
  proc.on("error", (err) => {
    console.log("[video-kb] transcribe: process error", err.message);
  ...
  const abortHandler = () => {
    console.log("[video-kb] transcribe: abort signal received");
    try { proc.kill("SIGTERM"); } catch { /* ignore */ }
    try { process.kill(proc.pid); } catch { /* ignore */ }
  };
  if (signal) {
    if (signal.aborted) { proc.kill("SIGTERM"); try { process.kill(proc.pid); } catch {} }
    else signal.addEventListener("abort", abortHandler, { once: true });
  }

  proc.on("close", (code) => {
    console.error("[whisper] process closed, code:", code, "resolved:", resolved);
    console.log("[video-kb] transcribe: process closed, exit code:", code);
    if (outputWatcher) clearInterval(outputWatcher);
    ...
    if (code !== 0 && code !== null) {
      if (outputDir && baseName) {
        const expectedJson = path.join(outputDir, `${baseName}.json`);
        if (fs.existsSync(expectedJson)) { console.log("[video-kb] transcribe: output exists, treating as success"); resolve(); return; }
      }
      reject(new Error(`${command} failed (exit ${code}): ${stderr.trim()}`)); return;
    }
    console.log("[video-kb] transcribe: success");
    resolve();
  });
}
