...
export function selectWebSearchEndpoint(endpoints = [], { secrets = null, env = process.env, fetchImpl = null } = {}) {
  ...
  for (const endpoint of ordered) {
    ...
    if (!apiKey) continue;
    const resolvedFetchImpl = typeof fetchImpl === "function" ? fetchImpl(endpoint) : fetchImpl;
    return {
      endpoint,
      providerId,
      adapter,
      ...
      fetchImpl: resolvedFetchImpl || null,
    };
  }
...
export function maybeInjectGatewayWebSearch(body, {
  ...
  const selected = selectWebSearchEndpoint(endpoints, { secrets, env, fetchImpl });
  if (!selected) {
