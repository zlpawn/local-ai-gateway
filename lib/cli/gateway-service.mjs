...
export function parseGatewayArgs(args) {
  ...
  if (command === "antigravity") {
    options.subcommand = args[1] && !args[1].startsWith("-") ? args[1] : "help";
  }
...
export function recoverMetadataFromHealth(port, health) {
  if (
    health?.ok
    && (health.service === "shrimp" || health.service === "local-ai-gateway")
...
export async function runGatewayCommand(options, io = console) {
  ...
  switch (options.command) {
    ...
    case "antigravity":
      return handleAntigravityCommand(context);
    case "logs":
...
Commands:
  ...
  antigravity  Antigravity OAuth login/status [login|status]
