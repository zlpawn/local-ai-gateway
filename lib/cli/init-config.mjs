...
export function detectDefaultDataDir(packageDir) {
  ...
  const preferred = path.join(os.homedir(), ".shrimp");
  const legacy = path.join(os.homedir(), ".local-ai-gateway");
  try {
    if (fsSync.existsSync(preferred) || !fsSync.existsSync(legacy)) return preferred;
  } catch {}
  return legacy;
}
