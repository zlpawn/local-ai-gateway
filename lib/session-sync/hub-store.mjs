...
export class HubStore {
  constructor(options = {}) {
    // Default to a stable user-level hub so every client/skill can find the same
    // sessions regardless of the gateway process working directory.
    // Override with baseDir/runtimeDir for tests or custom deployments.
    this.baseDir = options.baseDir
      || (options.runtimeDir ? path.join(options.runtimeDir, 'hub') : null)
      || process.env.LOCAL_AI_GATEWAY_HUB_DIR
      || path.join(os.homedir(), '.local-ai-gateway', 'hub');
