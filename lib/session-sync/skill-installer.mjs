...
export class SkillInstaller {
  ...
  static linkSkillToClient(skillName, client, enable, homeDir = os.homedir()) {
    ...
    if (enable) {
      ...
      if (isCopy) {
        ...
        return { client, linked: true, path: targetDir, sourceDir, mode: "copy" };
      }
      const symlinkType = process.platform === "win32" ? "junction" : "dir";
      try {
    ...
    try {
      ...
      if (isCopy) {
        ...
        return { client, linked: false, path: targetDir, mode: "copy" };
      }
  ...
  static unifyAllToCentral({ homeDir = os.homedir() } = {}) {
    ...
    for (const [name] of discovered) {
      ...
      if (!presentIn.central && !presentIn.antigravity && !presentIn.claude) continue;
      try {
  ...
  //
  // targets: { claude: bool, antigravity: bool, claudeDesktop3p: bool }
  ...
  static consolidateAndDispatch({
    ...
    const allSkills = SkillInstaller.listCentralSkills(homeDir)
    ...
    for (const skill of allSkills) {
      ...
      for (const [client, selected] of Object.entries(normalizedTargets)) {
        ...
        if (!toolMeta || toolMeta.isCentral) continue;
        ...
        try {
          ...
          if (selected && !isPresent) {
            ...
            SkillInstaller.linkSkillToClient(name, client, true, homeDir);
  ...
  static batchDeleteSkills(skillNames = [], homeDir = os.homedir()) {
    const results = [];
  ...
  static batchUnlinkSkills(skillNames = [], homeDir = os.homedir()) {
    const results = [];
    for (const rawName of skillNames) {
      const name = String(rawName || "").trim();
      if (!isSafeSkillName(name)) {
