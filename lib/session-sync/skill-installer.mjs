...
## 规则与命令
技能内置脚本脚本地址：\`lib/skills/grok-imagine/scripts/grok_imagine.mjs\`
...
export class SkillInstaller {
  ...
  static updateSymlinks(
    ...
    if (!fs.existsSync(targetCentralFile)) {
      SkillInstaller.installBaseSkill(path.dirname(targetCentralFile), skillName);
    }
    ...
    for (const [tool, dir] of Object.entries(toolPaths)) {
      ...
      try {
        if (shouldLink) {
          ...
          fs.symlinkSync(targetCentralFile, linkFile, symlinkType);
          results[tool] = true;
        } else {
          try {
            const lstat = fs.lstatSync(linkFile);
