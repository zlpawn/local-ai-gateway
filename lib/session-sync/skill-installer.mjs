...
export class SkillInstaller {
  static get standardSkillDirs() {
    const home = os.homedir();
    return [
      path.join(home, '.gemini/config/skills/session-sync'),
      path.join(home, '.claude/skills/session-sync'),
      path.join(home, '.codex/skills/session-sync')
    ];
  }

  static get defaultTargetDir() {
    return SkillInstaller.standardSkillDirs[0];
  }
  ...
  static install(targetDir) {
    ...
    // Install to all standard agent skill directories
    const installed = [];
    for (const dir of SkillInstaller.standardSkillDirs) {
      try {
    ...
    return installed[0] || SkillInstaller.install(SkillInstaller.defaultTargetDir);
  }
